...
describe('Enable incentives in target assets', () => {
  ...
  */
  before(async () => {
    ...
    await (
      await selfDestructContractV2.destroyAndTransfer(PSYS_GOVERNANCE_V2, {
        value: ethers.utils.parseEther('1'),
      })
    ).wait();
    ...
    await (
      await selfDestructContractV3.destroyAndTransfer(PSYS_SHORT_EXECUTOR, {
        value: ethers.utils.parseEther('1'),
      })
    ).wait();
  ...
  it('Proposal should be executed', async () => {
    const impersonatedGovernance = await ethers.provider.getSigner(PSYS_GOVERNANCE_V2);
    const impersonateExecutor = await ethers.provider.getSigner(PSYS_SHORT_EXECUTOR);
