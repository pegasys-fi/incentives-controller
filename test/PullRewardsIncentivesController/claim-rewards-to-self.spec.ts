...
makeSuite('pullRewardsIncentivesController claimRewardsToSelf tests', (testEnv) => {
  for (const {
    ...
    it(caseName, async () => {
      ...
      const { pullRewardsIncentivesController, psysToken, aDaiBaseMock } = testEnv;
      ...
      const destinationAddressBalanceBefore = await psysToken.balanceOf(destinationAddress);
      ...
      const destinationAddressBalanceAfter = await psysToken.balanceOf(destinationAddress);
